...
os.makedirs(f'marvel_multiverse/run_{timestamp}', exist_ok=True)

# Creating a txt file to save the output
output_file = open(f'marvel_multiverse/run_{timestamp}/marvel_sna_output_{timestamp}.txt', 'w')

marvel_data_edges = pd.read_csv('data/Marvel_SNA/hero-network.csv')

graph = nx.Graph()

# Add edges to the graph from the edges dataframe
graph = nx.from_pandas_edgelist(marvel_data_edges, source='hero1', target='hero2')
...
output_file.write("Marvel Social Network Analysis Output\n")
output_file.write("Processing Marvel data...\n\n")

# Save the degree and betweenness centrality to JSON format
deg_central_file = open(f'marvel_multiverse/run_{timestamp}/JSON_deg_central_marvel_{timestamp}.txt', 'w')
bet_central_file = open(f'marvel_multiverse/run_{timestamp}/JSON_bet_central_marvel_{timestamp}.txt', 'w')
...
fig.savefig(f'marvel_multiverse/run_{timestamp}/marvel_sna_graph_{timestamp}.png', dpi=300, bbox_inches='tight')
plt.show()
...
# format
with open(f'marvel_multiverse/run_{timestamp}/JSON_graph_marvel_{timestamp}.json', 'w') as f:
...
plt.savefig(f'marvel_multiverse/run_{timestamp}/marvel_sna_histogram_{timestamp}.png', dpi=300, bbox_inches='tight')
plt.show()
